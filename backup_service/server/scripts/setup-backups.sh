...
while getopts "m:" opt; do
    case $opt in
    m) mflag=true; CLIENT_IP=$OPTARG ;;
    r) rflag=true; REPLACE_MODE=$OPTARG ;; 
...
#res=`ssh -t ec2-user@$CLIENT_IP sudo grep FD_PASSWORD /etc/bacula/bacula-fd.conf 2> /dev/null`
#if [[ $res =~ .*FD_PASSWORD*. ]] 
#then
...
scp -q bacula-fd-temp.conf ec2-user@$CLIENT_IP:~/
ssh -t ec2-user@$CLIENT_IP sudo mv /home/ec2-user/bacula-fd-temp.conf /etc/bacula/bacula-fd.conf 2> /dev/null
echo -e ' ... \x1b[32;01mDONE\x1b[39;49;00m'


echo 'Restarting Bacula on Client      '
ssh -t ec2-user@$CLIENT_IP sudo service bacula-fd stop 2> /dev/null    # Should fail, but included just to be sure.
ssh -t ec2-user@$CLIENT_IP sleep 1 2> /dev/null
ssh -t ec2-user@$CLIENT_IP sudo service bacula-fd start 2> /dev/null
...
#ssh -t ec2-user@$CLIENT_IP sudo cp /home/ec2-user/home/ec2-user/* /home/ec2-user/
#ssh -t ec2-user@$CLIENT_IP sudo rm -rf /home/ec2-user/home